...
function activityIdForBooking(bookingId){const id=txt(bookingId);if(!id)throw new Error("booking_id_required");return `studio-booking-${id}`}
...
async function confirmBooking(bookingId,{confirmedBy}={}){
 ...
 if(plan.createActivity){activityId=activityIdForBooking(booking.id);const r=carRef(db,activityId),prior=await r.get();if(prior.exists){const data=prior.data()||{};if(txt(data.studioBookingId)!==txt(booking.id))throw new Error("booking_activity_id_collision");activity={id:r.id,...data,bookingStatus:"accepted",studioBookingId:txt(booking.id),studioId:txt(booking.studioId),branchId:txt(booking.branchId)||data.branchId||null,updatedAt:new Date().toISOString()};await r.set({bookingStatus:activity.bookingStatus,studioBookingId:activity.studioBookingId,studioId:activity.studioId,branchId:activity.branchId,updatedAt:activity.updatedAt},{merge:true})}else{activity={id:activityId,...compactBookingActivity(booking),createdAt:new Date().toISOString()};await r.set(activity)}}
...
module.exports={confirmBooking,proposeStudioChange,compactBookingActivity,activityIdForBooking};