...
test("原始應收應付與互抵後總額分開顯示", () => {
  ...
  assert.match(repository, /VIEW_SCHEMA_VERSION = 9/);
...
test("劇本費代收與外部店家付款使用獨立正式紀錄", () => {
  ...
  assert.match(feeController, /storeScriptFeeDetails/);
  assert.match(feeController, /storeExtraFeeDetails/);
  assert.match(feeController, /storePlayerPendingDetails/);
  assert.match(feeController, /storeReceivedDetails/);
