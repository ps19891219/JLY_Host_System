(function () {
  ...
  function switchMode(next){
    if(next===mode) return;
    mode=next;
    document.querySelectorAll(".matching-vote-slot-checkbox").forEach(c=>c.checked=mode==="exclude");
    document.querySelectorAll(".matching-vote-mode-button").forEach(b=>b.classList.toggle("is-active",b.dataset.mode===mode));
    document.getElementById("matchingVoteModeTitle").textContent="請保留你可以的時間";
    document.getElementById("matchingVoteModeDescription").textContent=mode==="exclude"?"已先幫你全部勾選；把少數不能配合的時間取消勾選即可。":"勾選你可以配合的時段即可。";
  }
