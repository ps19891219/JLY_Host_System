...
const pad=n=>String(n).padStart(2,'0'),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let currentFilter=null,currentMonth=new Date();
...
async function search(filter){
  ...
  rows=(await allActiveRows()).filter(r=>rowSearchText(r).includes(filter.keyword));
  if(title)title.textContent=`🔍 「${filter.label}」搜尋結果`;
  renderRows(rows,`找不到包含「${filter.label}」的班表。`,'search');
}
async function loadDefault(){currentFilter=null;updateMonthLabel();if(title)title.textContent='';host.innerHTML='<div class="empty">正在讀取班表…</div>';const mk=monthKey(),rows=(await V.loadMonth(mk)).filter(r=>r.status!=='cancelled');renderRows(rows,`目前 ${currentMonth.getMonth()+1} 月沒有排班。`,'default')}
...
window.JLYWorkScheduleDateSearch={refresh,loadDefault,monthKey,searchCurrent:()=>currentFilter?search(currentFilter):loadDefault(),parseFilter};
