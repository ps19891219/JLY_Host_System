...
function groups(rows){const m=new Map();(rows||[]).forEach(r=>{const k=groupKey(r);if(!m.has(k))m.set(k,{key:k,date:r.date,workId:r.workId||'',workName:r.workName||'未命名工作',hostName:r.hostName||'',startTime:r.startTime||'',endTime:r.endTime||'',endDate:r.endDate||r.date,studioName:r.studioName||'',rows:[]});m.get(k).rows.push(r)});return[...m.values()].map(g=>({...g,rows:sortRoles(g.rows)})).sort((a,b)=>(a.date+a.startTime+a.workName).localeCompare(b.date+b.startTime+b.workName))}
