...
function appendToBatch(batch,shiftRef,before,after,context={}){
 ...
 if(confirmationInvalidated&&shiftRef){const states={...(before?.assignmentConfirmationByPerson||{}),...(after?.assignmentConfirmationByPerson||{})},confirmed=Object.keys(states).filter(id=>states[id]==='confirmed'),assignedNow=ids(after?.assignedPersonIds??after?.personIds??before?.assignedPersonIds??before?.personIds??[]),existingTentative=ids(after?.tentativePersonIds??before?.tentativePersonIds??[]),reconfirm=ids([...confirmed,...existingTentative]),nextAssigned=assignedNow.filter(id=>!reconfirm.includes(id)),nextTentative=ids([...existingTentative,...reconfirm]),nextStates={...states};reconfirm.forEach(id=>nextStates[id]='invalidated');const slotSource=Array.isArray(after?.staffSlots)?after.staffSlots:(Array.isArray(before?.staffSlots)?before.staffSlots:[]),peopleSource=Array.isArray(after?.people)?after.people:(Array.isArray(before?.people)?before.people:[]),nextSlots=slotSource.map(s=>reconfirm.includes(String(s?.personId||''))?{...s,personId:''}:s),nextPeople=peopleSource.filter(p=>!reconfirm.includes(String(p?.personId||p?.id||'')));batch.set(shiftRef,{assignedPersonIds:nextAssigned,personIds:nextAssigned,tentativePersonIds:nextTentative,staffSlots:nextSlots,people:nextPeople,assignmentConfirmationByPerson:nextStates,assignmentConfirmationStatus:'invalidated',assignmentConfirmationUpdatedAt:ts()},{merge:true});}\n batch.set(eventRef,{shiftId,workId,studioId,workName:after?.workName||before?.workName||'',roleName:after?.roleName||before?.roleName||'',date:after?.date||before?.date||'',startTime:after?.startTime||before?.startTime||'',endTime:after?.endTime||before?.endTime||'',classification:summary.classification,affectedPersonIds:summary.affectedPersonIds,assignmentDelta:summary.assignmentDelta,calendarPlan:summary.calendarPlan,notificationPlan:summary.notificationPlan,notificationStatus:summary.notificationPlan.length?'pending':'none',personalCalendarStatus:hasCalendar?'waiting_person_calendar':'none',source:context.source||'work_schedule',actorPersonId:String(context.actorPersonId||''),conflictAcknowledged:Boolean(context.conflictAcknowledged),conflictCount:Number(context.conflictCount||0),createdAt:ts(),lastError:''});
