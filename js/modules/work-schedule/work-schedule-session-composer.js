(function(){
'use strict';
if(window.__JLYWorkScheduleSessionComposerInitialized)return;
window.__JLYWorkScheduleSessionComposerInitialized=true;

const db=window.db||firebase.firestore();
const works=db.collection('workScheduleWorks');
const shifts=db.collection('workShifts');
const V=window.JLYWorkScheduleReadView;
const L=window.JLYWorkScheduleLifecycle;
...
async function ensurePeople(){
  ...
  try{people=await window.JLYMemberPickerData.loadPersonDirectory()}catch(_){people=[]}
  return people;
}
async function loadWork(){
  if(!activeWorkId)return null;
  const d=await works.doc(activeWorkId).get();
  activeWork=d.exists?{id:d.id,...d.data()}:null;
  return activeWork;
}
...
function effectiveEnd(start,end){return txt(end)||addMinutes(start,60)}
function endDate(date,start,end){
  if(!date)return date;
  const resolved=effectiveEnd(start,end);
  if(resolved&&start&&resolved<=start){
...
function ensureDialog(){
  let dlg=$('workSessionComposerDialog');
  if(dlg)return dlg;
  ...
  $('workSessionAddTime').onclick=()=>{timeSlots.push({start:'19:00',end:''});renderTimes();renderSessions()};
  $('workSessionCalendarPrev').onclick=()=>{calendarCursor=new Date(calendarCursor.getFullYear(),calendarCursor.getMonth()-1,1);renderCalendar()};
...
function yearOptions(){
  const now=new Date().getFullYear(),years=new Set();
  for(let y=now-2;y<=now+10;y++)years.add(y);
  years.add(calendarCursor.getFullYear());
  for(const d of selectedDates)years.add(Number(String(d).slice(0,4)));
  return[...years].filter(Boolean).sort((a,b)=>a-b);
}
function renderCalendar(){
  const host=$('workSessionCalendar');
  if(!host)return;
  const y=calendarCursor.getFullYear(),m=calendarCursor.getMonth(),yearSel=$('workSessionCalendarYear'),monthSel=$('workSessionCalendarMonth');
  yearSel.innerHTML=yearOptions().map(v=>`<option value="${v}"${v===y?' selected':''}>${v} 年</option>`).join('');
  monthSel.innerHTML=Array.from({length:12},(_,i)=>`<option value="${i+1}"${i===m?' selected':''}>${i+1} 月</option>`).join('');
  const first=new Date(y,m,1),last=new Date(y,m+1,0),offset=(first.getDay()+6)%7,cells=[];
  for(let i=0;i<offset;i++)cells.push('<span class="session-composer-calendar-empty" aria-hidden="true"></span>');
  for(let day=1;day<=last.getDate();day++){
    const d=new Date(y,m,day),key=isoLocal(d),on=selectedDates.has(key);
    cells.push(`<button type="button" data-session-date="${key}" class="${on?'selected':''}" aria-pressed="${on}"><span>${day}</span></button>`);
  }
  host.innerHTML=cells.join('');
  host.querySelectorAll('[data-session-date]').forEach(b=>b.onclick=()=>{const v=b.dataset.sessionDate;selectedDates.has(v)?selectedDates.delete(v):selectedDates.add(v);renderCalendar();renderSessions()});
  $('workSessionSelectedCount').textContent=`已選 ${selectedDates.size} 天`;
}
function renderTimes(){
  const host=$('workSessionTimeRows');
...
function togglePerson(date,ti,rk,pid){
  const k=assignmentKey(date,ti,rk),arr=selectedIds(date,ti,rk).slice(),i=arr.indexOf(pid);
...
function updateSummary(){
  const sessions=selectedDates.size*timeSlots.length,selected=Object.values(assignments).reduce((n,a)=>n+a.length,0);
  $('workSessionSummary').textContent=sessions?`共 ${sessions} 場，已選 ${selected} 個人次。未填結束時間會自動以 1 小時計算；明確填寫可超過 10 小時並支援跨日。`:'尚未選擇場次。';
}
async function open(){
  await Promise.all([loadWork(),ensurePeople()]);
  if(!activeWork)return alert('找不到目前的劇本設定。');
  if(!(activeWork.roles||[]).some(r=>txt(r.name)))return alert('請先設定至少一個角色。');
  ensureDialog();
...
function payload(role,date,start,end,endWasExplicit,note,hostName,ids,refId){
  ...
  return{
    ...
    studioName:activeWork.studioName||'',
    studioId:activeWork.studioId||activeWork.organizationId||'',
    organizationId:activeWork.organizationId||activeWork.studioId||'',
    hostName,
    roleName:role.name,
    rolePoolId:role.id||role.name,
    eligiblePersonIds:roleIds(role),
    rolePoolsSnapshot:(activeWork.roles||[]).map(x=>({id:x.id,name:x.name,eligiblePersonIds:roleIds(x)})),
    date,
    monthKey:date.slice(0,7),
    startTime:start,
    endTime:end,
    endDate:endDate(date,start,end),
    endTimeExplicit:endWasExplicit,
...
async function save(e){
  e.preventDefault();
  if(!L||!V)return alert('排班模組尚未載入完成，請重新整理後再試。');
  const dates=[...selectedDates].sort();
  if(!dates.length)return alert('請先選日期。');
  ...
  for(const date of dates){
    for(let ti=0;ti<timeSlots.length;ti++){
      ...
      for(let ri=0;ri<roles.length;ri++){
        ...
        const existing=(monthRows.get(date.slice(0,7))||[]).find(r=>rowMatches(r,date,t.start,resolvedEnd,role));
        if(existing){
          const ok=confirm(`${fmtDate(date)} ${t.start}–${resolvedEnd}${endWasExplicit?'':'（預設 1 小時）'} ${role.name} 已有排班。\n\n確定要用這次選的人員覆蓋嗎？`);
          if(!ok)continue;
        }
        const ref=existing?shifts.doc(existing.id):shifts.doc(),before=existing?{...existing}:{};
        const after={id:ref.id,...payload(role,date,t.start,resolvedEnd,endWasExplicit,note,hostName,ids,ref.id)};
        if(existing){
          after.createdAt=existing.createdAt||null;
          const idx=working.findIndex(x=>String(x.id)===String(existing.id));
          if(idx>=0)working.splice(idx,1);
        }
        const summary=L.summarize(before,after);
        conflicts.push(...L.findConflicts({personIds:summary.assignmentDelta.added,candidate:after,rows:working,ignoreShiftIds:existing?[existing.id]:[]}));
        working.push(after);
        changes.push({ref,before,after,exists:!!existing});
      }
    }
  }
  if(!changes.length)return alert('請至少替一場選擇工作人員。');
  let ack=false;
  if(conflicts.length){if(!confirm(L.conflictMessage(conflicts,{personLabel:id=>personName(id)})))return;ack=true}
  try{
    for(let i=0;i<changes.length;i+=180){
      const batch=db.batch();
      for(const c of changes.slice(i,i+180)){
        const data={...c.after};delete data.id;
        if(c.exists){delete data.createdAt;batch.update(c.ref,data)}else{data.createdAt=firebase.firestore.FieldValue.serverTimestamp();batch.set(c.ref,data)}
        L.appendToBatch(batch,c.ref,c.before,c.after,{source:'session_composer',conflictAcknowledged:ack,conflictCount:conflicts.length});
      }
      await batch.commit();
    }
