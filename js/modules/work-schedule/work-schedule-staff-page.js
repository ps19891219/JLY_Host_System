...
function groupFromRows(list){
  const map=new Map();
  list.forEach(r=>{
    const key=groupKey(r);
    if(!map.has(key))map.set(key,{key,first:r,rows:[]});
    map.get(key).rows.push(r);
  });
  return [...map.values()].map(g=>{
    g.rows.sort((a,b)=>String(a.id).localeCompare(String(b.id)));
    g.first=g.rows[0]||g.first;
    g.stableId=g.rows[0]?.id||g.key;
    g.mineRows=g.rows.filter(r=>r.isMine);
    g.isMine=g.mineRows.length>0;
    return g;
  });
}
...
function formatOwnRoles(group){
  return group.mineRows.map(r=>r.roleName||'工作人員').filter(Boolean).join('、');
}
function render(){
  ...
  host.innerHTML=list.map(g=>{
    const r=g.first;
    const selectable=view==='mine'&&g.isMine;
    const checked=selectable&&selected.has(g.key);
    const endText=r.endTime?`${r.startTime}–${r.endDate!==r.date?'翌日 ':''}${r.endTime}`:`${r.startTime}（預設 1 小時）`;
    return `<article class="card staff-shift-card">
      <div class="staff-shift-head">${selectable?`<label><input type="checkbox" data-group="${html(g.key)}" ${checked?'checked':''}> 選取</label>`:''}<strong>${html(r.date)} ${html(endText)}</strong></div>
      <h3>${html(r.workName||'未命名工作')}${r.hostName?`｜主揪：${html(r.hostName)}`:''}</h3>
      <p>${html(formatPeople(g))}</p>
      ${g.isMine?`<small>✓ 你的班${formatOwnRoles(g)?`：${html(formatOwnRoles(g))}`:''}</small>`:''}
    </article>`;
  }).join('');
  host.querySelectorAll('[data-group]').forEach(input=>input.onchange=()=>{
    input.checked?selected.add(input.dataset.group):selected.delete(input.dataset.group);
  });
}
...
async function eventIdForGroup(stableId){
...
function plusOneHour(date,time){
  ...
  const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Taipei',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(d);
...
function resource(group,eventId){
  ...
  return {
    ...
    description:[ownRoles?`我的角色：${ownRoles}`:'',row.hostName?`主揪：${row.hostName}`:'',row.note||'','JLY Work Schedule'].filter(Boolean).join('\n'),
    location:row.studioName||'',
    start:{dateTime:start,timeZone:'Asia/Taipei'},
    end:{dateTime:end,timeZone:'Asia/Taipei'},
    extendedProperties:{private:{source:'JLY',sourceModule:'work-schedule-staff',workScheduleGroupId:String(group.stableId)}}
  };
}
...
async function upsertGroup(token,group){
  if(!group.isMine)throw new Error('只能匯入自己的班。');
  const eventId=await eventIdForGroup(group.stableId);
  const base='https://www.googleapis.com/calendar/v3/calendars/primary/events';
  const body=JSON.stringify(resource(group,eventId));
...
async function deleteGroup(token,group){
  const eventId=await eventIdForGroup(group.stableId);
...
async function syncSelected(){
  const mine=groups().filter(g=>g.isMine&&selected.has(g.key));
  if(!mine.length)return alert('請先在「只看我的」勾選要匯入的班。');
  const token=await window.JLYCalendarAuth.requestAccessToken();
  for(const group of cancelledGroups()){
    try{await deleteGroup(token,group)}catch(e){console.warn('取消班表同步清理失敗',group.key,e)}
  }
  let ok=0;
  for(const group of mine){await upsertGroup(token,group);ok++}
  alert(`已匯入／更新 ${ok} 場自己的班到 Google Calendar。重複操作會更新同一筆，不會另外新增。`);
}

function bind(){
  document.querySelectorAll('[data-staff-view]').forEach(b=>b.onclick=()=>{view=b.dataset.staffView;selected.clear();render()});
  $('staffSelectAll').onclick=()=>{groups().filter(g=>g.isMine).forEach(g=>selected.add(g.key));render()};
