(function () {
  ...
  function settlementRows(item, model) {
    ...
    return (item.splits || []).map(split => {
      const mine=split.personId===model.currentPersonId,receiver=item.paidBy===model.currentPersonId,name=model.memberNames.get(split.personId)||split.displayName||"成員";
      let label="尚未付款",buttons="";
      if(split.settlementStatus==="settled")label="已結清";
      else if(split.settlementStatus==="payment_claimed"){label="已付款，待確認";if(mine)buttons=`<button data-action="withdraw">撤回</button>`;if(receiver)buttons=`<button data-action="confirm">確認收到</button><button data-action="reject">尚未收到</button>`;}
      else if(split.settlementStatus==="settlement_rejected")label="付款被退回";
      if(mine&&["payment_due","settlement_rejected"].includes(split.settlementStatus))buttons=`<button data-action="claim">我已付款</button>`;
