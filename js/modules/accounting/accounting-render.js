(function () {
  ...
  function settlementRows(item, model) {
    ...
    return (item.splits || []).map(split => {
      ...
      else if(split.settlementStatus==="payment_claimed"){label="已付款，待確認";if(mine)buttons=`<button data-action="withdraw">撤回</button>`;if(receiver||manager)buttons+=`<button data-action="confirm">${receiver?"確認收到":"主揪確認收款"}</button><button data-action="reject">尚未收到</button>`;}
      ...
      if(["payment_due","settlement_rejected"].includes(split.settlementStatus)){
        if(mine)buttons+=`<button data-action="claim">我已付款</button>`;
        if(manager&&!mine)buttons+=`<button data-action="manager_claim">主揪代登已付款</button>`;
        if(receiver||manager)buttons+=`<button data-action="receiver_settle">${receiver?"標記已收款":"主揪標記已收款"}</button>`;
      }
