(function (root, factory) {
  ...
  function personalAccountingProjection(items,personId,canonicalize){
    const resolve=typeof canonicalize==="function"?value=>text(canonicalize(text(value))):text,id=resolve(personId),byPerson=new Map();
    (Array.isArray(items)?items:[]).forEach(item=>{const from=resolve(item&&item.fromPersonId),to=resolve(item&&item.toPersonId),value=Math.round(Number(item&&item.amount)||0);if(!from||!to||from===to||value<=0)return;let other="",payable=0,receivable=0;if(from===id){other=to;payable=value;}else if(to===id){other=from;receivable=value;}else return;const current=byPerson.get(other)||{personId:other,payable:0,receivable:0};current.payable+=payable;current.receivable+=receivable;byPerson.set(other,current);});
    const people=[...byPerson.values()].map(item=>{const net=item.receivable-item.payable;return{...item,net,direction:net>0?"receivable":net<0?"payable":"settled",amount:Math.abs(net)};}).filter(item=>item.amount>0).sort((a,b)=>a.personId.localeCompare(b.personId)),payableTotal=people.filter(item=>item.direction==="payable").reduce((sum,item)=>sum+item.amount,0),receivableTotal=people.filter(item=>item.direction==="receivable").reduce((sum,item)=>sum+item.amount,0),net=receivableTotal-payableTotal;
