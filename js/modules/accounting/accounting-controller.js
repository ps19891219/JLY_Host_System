(function () {
  "use strict";
  let mountedCarId = "", loading = false, detailedTransactions = null, detailedCarId = "", detailLastDocument = null, detailHasMore = false;
  ...
  async function mount() {
    ...
    try {
      ...
      section.innerHTML=renderer.buildDashboardHtml({members,currentPersonId,managerPersonId,memberNames,transactions:displayedTransactions,pendingActions:dashboard.pendingActions,counts:dashboard.pendingCounts||counts(dashboard.pendingActions),netSettlement:data.netSettlementFromBalances(dashboard.balanceByPerson),detailMode:Boolean(detailedTransactions),detailHasMore,getFilterState:item=>data.transactionFilterState(item,currentPersonId)});
