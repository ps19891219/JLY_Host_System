(function () {
  ...
  async function mount() {
    ...
    try {
      ...
      section.innerHTML=renderer.buildDashboardHtml({members,currentPersonId,managerPersonId,memberNames,transactions:dashboard.transactions,pendingActions:dashboard.pendingActions,counts:counts(dashboard.pendingActions),netSettlement:data.calculateNetSettlement(dashboard.transactions),getFilterState:item=>data.transactionFilterState(item,currentPersonId)});
      ...
      window.JLYAccountingActions.bind({section,transactions:dashboard.transactions,onSave:async input=>{if(!currentPersonId)throw new Error("identity_required");if(!members.some(member=>member.personId===currentPersonId))throw new Error("activity_membership_required");const canUuid=window.crypto&&typeof window.crypto.randomUUID==="function";const transactionId=canUuid?`web-${window.crypto.randomUUID()}`:`web-${Date.now()}`;const transaction=data.buildQuickTransaction({...input,transactionId,activityId:carId,createdBy:currentPersonId},new Date().toISOString());await repository.createQuickTransaction(transaction,String(car.ownerId||currentPersonId));},onSplit:async input=>{const entry=dashboard.transactions.find(item=>item.transactionId===input.transactionId);const selected=input.personIds.map(id=>members.find(member=>member.personId===id)).filter(Boolean);const splits=input.mode==="custom"?data.buildCustomSplits(selected,input.amounts,entry.amount,entry.paidBy):data.buildEqualSplits(selected,entry.amount,entry.paidBy);await repository.completeSplit(carId,entry.transactionId,splits,currentPersonId,managerPersonId);},onSettlement:async input=>{const entry=dashboard.transactions.find(item=>item.transactionId===input.transactionId),split=(entry.splits||[]).find(item=>item.splitId===input.splitId),next=data.transitionSettlement(split,input.action,currentPersonId,entry.paidBy,new Date().toISOString(),managerPersonId);await repository.saveSettlement(carId,entry.transactionId,split.splitId,next,currentPersonId);},onReload:()=>{loading=false;mount();}});
      mountedCarId=carId;
