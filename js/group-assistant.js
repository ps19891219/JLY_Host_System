(function () {
  ...
  function renderActivityAccountingExperience(data){
    ...
    root.innerHTML=`${personId?"":'<div class="notice">請先使用 LINE 登入，才能查看及處理自己的帳務。</div>'}<nav class="line-accounting-tabs">${[["overview","總覽"],["entries","逐筆明細"],["people","人物帳務"],["pending","待處理"],["history","歷史"]].map(([id,label])=>`<button type="button" data-line-accounting-tab="${id}">${label}</button>`).join("")}</nav><section data-line-accounting-panel="overview"><div class="totals"><article><span>活動總額</span><b>${money(accounting.totalExpense)}</b></article><article><span>我的待處理</span><b>${pending.length}</b></article></div><p class="settlement-note">付款、還款與核銷不會重複計入活動總額。</p></section><section data-line-accounting-panel="entries" hidden>${txRows}</section><section data-line-accounting-panel="people" hidden><h3>應付</h3>${pairRows(payable,"pay")||'<p class="empty">目前沒有應付</p>'}<h3>應收</h3>${pairRows(receivable,"receive")||'<p class="empty">目前沒有應收</p>'}</section><section data-line-accounting-panel="pending" hidden>${pendingRows}</section><section data-line-accounting-panel="history" hidden>${historyRows}</section>`;
