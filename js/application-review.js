...
console.log("application-review.js V3 已成功載入！");
...
async function loadOwnedCars(ownerIds) {
  ...
  for (const ownerId of ids) {
    ...
    (Array.isArray(rows) ? rows : []).forEach(function (car) {
...
function setReviewType(type) {
  currentReviewType = type === "dm" ? "dm" : "player";
  renderReviewList();
}

function buildPlayerItem(app) {
  const name = reviewEscape(
    app.name || app.playerName || "未命名玩家"
  );
  const position = reviewEscape(
    app.role || app.position || "不限"
  );
  const cross = app.isCrossPlay === true ? "／反串" : "";
  const claim = app.claimType === "existing_person"
    ? "認領既有玩家｜" + reviewEscape(
        app.targetPlayerName || app.name || app.playerName || "既有玩家"
      )
    : "新增玩家報名";
...
function buildDmItem(app) {
  ...
  const claim = app.claimType === "existing_slot"
    ? "認領 " + reviewEscape(
        (app.targetStaffLabel ? app.targetStaffLabel + "｜" : "") +
        (app.targetStaffName || "既有 DM")
      )
    : "新增為本場 DM";
...
function renderReviewList() {
  ...
  const key = currentReviewType === "dm"
    ? "dmApplications"
    : "playerApplications";

  const rows = currentReviewSummary.cars.filter(function (item) {
  ...
  if (rows.length === 0) {
    list.innerHTML = `
      <div class="card">
        <h3>${currentReviewType === "dm" ? "🎭 DM" : "🎮 玩家"}</h3>
        <p>目前沒有待審核申請。</p>
      </div>
    `;
    return;
  ...
  list.innerHTML = rows.map(function (item) {
    ...
    const apps = item[key];
    const body = apps
      .map(currentReviewType === "dm" ? buildDmItem : buildPlayerItem)
      .join("");
    const carId = encodeURIComponent(reviewText(car.id));

    return `
      <div class="card">
        <h3>${reviewEscape(car.scriptName || car.name || "未命名劇本")}</h3>
        <p>${apps.length} 筆待審核</p>
        ${body}
        <button
          type="button"
          onclick="location.href='car-detail.html?id=${carId}'"
        >
          前往處理
        </button>
      </div>
...
async function loadRegistrationReview() {
  ...
  try {
    ...
    if (!window.JLYPendingActions) {
      throw new Error("Pending Actions 模組尚未載入");
    }

    const cars = await loadOwnedCars(ownerIds);

    currentReviewSummary = window.JLYPendingActions
      .buildRegistrationSummary(cars);

    const playerCount = document.getElementById("playerReviewCount");
    const dmCount = document.getElementById("dmReviewCount");

    if (playerCount) {
      playerCount.textContent = String(currentReviewSummary.playerCount);
    }
    if (dmCount) {
      dmCount.textContent = String(currentReviewSummary.dmCount);
    }
    ...
    if (list) {
      list.innerHTML = `
        <div class="card">
          <h3>報名審核讀取失敗</h3>
          <p>${reviewEscape(error.message || "未知錯誤")}</p>
        </div>
      `;
    }
  }
