...
(function () {
  ...
  function normalizeExistingSeatSummary(summary) {
    if (!summary || typeof summary !== "object") return null;
    const normalized = {
      totalSeatCount: Math.max(0, Number(summary.totalSeatCount) || 0),
      occupiedSeatCount: Math.max(0, Number(summary.occupiedSeatCount) || 0),
      maleTotal: Math.max(0, Number(summary.maleTotal) || 0),
      maleOccupied: Math.max(0, Number(summary.maleOccupied) || 0),
      femaleTotal: Math.max(0, Number(summary.femaleTotal) || 0),
      femaleOccupied: Math.max(0, Number(summary.femaleOccupied) || 0),
      flexibleTotal: Math.max(0, Number(summary.flexibleTotal) || 0),
      flexibleOccupied: Math.max(0, Number(summary.flexibleOccupied) || 0),
      waitingCount: Math.max(0, Number(summary.waitingCount) || 0)
    };
    const hasSeatData = normalized.totalSeatCount > 0 || normalized.maleTotal > 0 || normalized.femaleTotal > 0 || normalized.flexibleTotal > 0;
    return hasSeatData ? normalized : null;
  }

  function buildSeatSummary(car) {
    const source = car && typeof car === "object" ? car : {};
    const slots = Array.isArray(source.slots) ? source.slots : [];
    if (slots.length === 0) return normalizeExistingSeatSummary(source.seatSummary);

    const activePlayerIds = new Set(
      (Array.isArray(source.players) ? source.players : [])
        .filter(function (player) {
          const status = text(player && player.status);
          return !["已取消", "取消", "cancelled", "canceled"].includes(status);
        })
        .map(function (player) {
          return text(player && (player.playerId || player.id || player.profileId || player.applicationId));
        })
        .filter(Boolean)
    );
    ...
    slots.forEach(function (slot) {
      ...
      const isOccupied = Boolean(playerId) && (activePlayerIds.size === 0 || activePlayerIds.has(playerId));
  ...
  function compactCar(car, viewerIdentityIds) {
    ...
    const isHost = Boolean(ownerId) && identitySet.has(ownerId) || (!hasFormalViewerIdentity && (source.isHost === true || text(source.role) === "host" || text(source.ownerType) === "self"));
    const isPlayer = !isHost && ((!hasFormalViewerIdentity && source.isPlayer === true) || players.some(function (player) {
      if (!player || !identitySet.has(text(player.playerId))) return false;
      return !["已取消", "取消", "cancelled", "canceled"].includes(text(player.status));
    }));
    ...
    return {
      ...
      isHost,
      isPlayer,
      role: isHost ? "host" : (isPlayer ? "player" : ""),
      ownerType: isHost ? "self" : "",
  ...
  function sortCars(cars) {
    return [...(Array.isArray(cars) ? cars : [])].sort(function (a, b) {
      function group(car) {
        if (isEnded(car)) return 2;
        if (isPlanning(car)) return 1;
        return 0;
      }
      const aGroup = group(a), bGroup = group(b);
  ...
  function buildView(options) {
    ...
    const identityIds = Array.from(new Set([viewerId, ...(Array.isArray(settings.identityIds) ? settings.identityIds : [])].map(text).filter(Boolean)));
    const cars = sortCars((Array.isArray(settings.cars) ? settings.cars : []).map(function (car) {
      return compactCar(car, identityIds);
    }).filter(function (car) {
      return car.id && (car.isHost || car.isPlayer);
    }));
    return {
  ...
  function selectForUi(view, options) {
    ...
    if (tab === "active") {
      cars = cars.filter(function (car) { return !isEnded(car) && !isPlanning(car); });
    ...
    if (keyword) {
      cars = cars.filter(function (car) {
        const searchable = [car.scriptName, car.gameDate, car.gameTime, car.locationName, car.location, car.organizerName, car.studioName, car.dmName, ...(car.tags || []), ...(car.scriptTags || [])].join(" ").toLowerCase();
  ...
  function applyMutationToView(view, beforeCar, afterCar) {
    ...
    const identityIds = Array.from(new Set([viewerId, ...(Array.isArray(view.identityIds) ? view.identityIds : [])].map(text).filter(Boolean)));
    const carId = text((afterCar && (afterCar.id || afterCar.carId)) || (beforeCar && (beforeCar.id || beforeCar.carId)));
    if (!carId) return view;
    let cars = (Array.isArray(view.cars) ? view.cars : []).filter(car => text(car && car.id) !== carId);
    if (afterCar) {
