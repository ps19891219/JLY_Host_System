...
function buildRecruitmentView(studioId,activities=[],domain){
 ...
 const items=arr(activities).filter(a=>txt(a&&a.studioId||a&&a.organizationId)===id&&domain.recruitmentVisible(a)).map(compactRecruitment);
...
function applyRecruitmentMutation(view,activity,domain){const row=compactRecruitment(activity);return applyById(view,'activities',row,!!row.activityId&&row.studioId===txt(view&&view.studioId)&&domain.recruitmentVisible(activity));}
